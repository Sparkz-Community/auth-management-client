  ...
  <q-card>
    ...
    <q-slide-transition>
      <template v-if="adding === 'address'">
        <q-card-section>
          <vue-tel-input :style="{border: isValid ? '1px solid green' : (!isValid && inputVal ? '1px solid red' : '')}" :preferredCountries="preferredCountries" @update:modelValue="handlePhone"></vue-tel-input>
        </q-card-section>
      </template>
    </q-slide-transition>
    <q-card-section>
